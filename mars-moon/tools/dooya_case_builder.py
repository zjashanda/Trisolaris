...
class RequirementCaseBuilder:
    ...
    def build_base_cases(self) -> list[dict[str, Any]]:
        ...
        cases = [
            ...
            case(
                ...
                "P1",
                [wake_action(self.wake_word), wait_action(timeout_s, expect_tone_id="TONE_ID_2", expect_markers=["TIME_OUT"], measure_timeout=True)],
    ...
    def build_vol_cases(self) -> list[dict[str, Any]]:
        ...
        down_steps = max(default_level - 1, 1)
        up_steps = max(level_count - default_level, 1)
        factory_word, factory_tone_id, factory_tone_text, factory_protocol = self.factory_entry_action()
        reset_to_default_volume_actions = [
            say_action(
                factory_word,
                expect_tone_id=factory_tone_id,
                expect_tone_text=factory_tone_text,
                expect_send_protocol=factory_protocol,
                require_protocol_and_tone=False,
            ),
            wait_action(2.0),
        ]
        return [
            ...
            case(
                ...
                "默认音量到最大档位步数正确",
                "设备处于默认音量",
                f"从默认音量开始重复执行【{increase_word}】，统计到最大音量前的步进次数并检查上边界提示",
                "默认音量到最大档位的步数符合需求，达到最大音量后播报上边界提示",
                "",
                norm_text(rules.get("upper_boundary_tone"), "TONE_ID_4"),
                self.tone_text(norm_text(rules.get("upper_boundary_tone"), "TONE_ID_4")),
                "P1",
                [
                    *reset_to_default_volume_actions,
                    {
                        ...
                        "boundary_tone_id": norm_text(rules.get("upper_boundary_tone"), "TONE_ID_4"),
                        "extra_boundary_repeats": 1,
                    },
                ],
                self.requirement_ids_direct("CFG-002", "CFG-003", "CFG-004"),
                "boundary",
            ),
            case(
                self.next_case_id("VOL"),
                ...
                "默认音量到最小档位步数正确",
                "设备处于默认音量",
                f"从默认音量开始重复执行【{decrease_word}】，统计到最小音量前的步进次数并检查下边界提示",
                "默认音量到最小档位的步数符合需求，达到最小音量后播报下边界提示",
                "",
                norm_text(rules.get("lower_boundary_tone"), "TONE_ID_5"),
                self.tone_text(norm_text(rules.get("lower_boundary_tone"), "TONE_ID_5")),
                "P1",
                [
                    *reset_to_default_volume_actions,
                    {
                        ...
                        "step_tone_id": step_tone_id,
                        "boundary_tone_id": norm_text(rules.get("lower_boundary_tone"), "TONE_ID_5"),
                        "extra_boundary_repeats": 1,
                    },
                ],
                self.requirement_ids_direct("CFG-002", "CFG-003", "CFG-005"),
                "boundary",
            ),
            case(
                self.next_case_id("VOL"),
                ...
                "恢复出厂后音量恢复默认",
                "设备音量已被改为非默认值",
                f"先把音量改到非默认档位，再执行恢复出厂，随后重复执行【{decrease_word}】直到最小音量，检查步数是否回到默认档位",
                "恢复出厂后音量恢复默认档位，到最小音量前的步数与默认音量一致",
                "",
                norm_text(rules.get("lower_boundary_tone"), "TONE_ID_5"),
                self.tone_text(norm_text(rules.get("lower_boundary_tone"), "TONE_ID_5")),
                "P1",
                ...
                self.requirement_ids_direct("FACT-002", "CFG-003"),
                "state_transition",
            ),
        ]
    ...
    def build_power_cases(self) -> list[dict[str, Any]]:
        ...
        alternate_wakes = list(wake_rules.get("candidate_wake_words", []))
        ...
        wake_entry_actions = self.setting_entry_actions(wake_entry_word, wake_entry_tone_id, wake_entry_tone_text)
        changed_wake_row = self.word_row(changed_wake)
        ...
        expected_down_steps = retained_down_steps if power_rules.get("volume_retained_after_power_cycle") else max(default_level - 1, 1)
        ...
        return [
            ...
            case(
                ...
                f"先进入【{wake_entry_word}】并切换到候选唤醒词【{changed_wake}】，再执行上下电，然后检查当前候选唤醒词和其他候选词的生效边界",
                "掉电后唤醒词保持行为符合配置：当前候选唤醒词按需求保持，其他候选词不误生效",
                "",
                "TONE_ID_0",
                self.tone_text("TONE_ID_0"),
                "P1",
                [
                    *wake_entry_actions,
                    say_action(
                        ...
                        expect_log_values=self.wakeword_log_expectations(changed_wake),
                        advisory_log_values=self.config_saved_advisory_logs(),
                        require_protocol_and_tone=False,
                        auto_wake=False,
                    ),
                    manual_power_cycle_action(note="执行上下电循环，校验唤醒词掉电保持行为"),
                    wake_action(changed_wake if power_rules.get("wake_word_retained_after_power_cycle") else self.wake_word),
            ...
            case(
                ...
                "掉电后音量保持规则正确",
                "设备处于默认音量且支持硬断电",
                f"先执行【{factory_word}】恢复默认音量，再执行【{decrease_word}】把音量改成非默认，然后上下电，最后继续执行【{decrease_word}】直到最小音量，检查剩余步数是否符合掉电保持配置",
                ...
                "P1",
                [
                    say_action(factory_word, expect_tone_id=factory_tone_id, expect_tone_text=factory_tone_text, expect_send_protocol=factory_protocol, require_protocol_and_tone=False),
                    wait_action(2.0),
                    say_action(
                        decrease_word,
                        expect_tone_id=step_tone_id,
                        expect_tone_text=self.tone_text(step_tone_id),
                        advisory_log_values={"configSaved": ["save config success"]},
                        observe_s=5.0,
                        require_protocol_and_tone=False,
                    ),
    ...
    def build_factory_cases(self) -> list[dict[str, Any]]:
        alternate_wakes = list(self.behavior_rules.get("wake_word_setting", {}).get("candidate_wake_words", []))
        changed_wake = alternate_wakes[0] if alternate_wakes else "客厅窗帘"
        changed_wake_row = self.word_row(changed_wake)
        changed_wake_tone_id = self.setting_success_tone_id()
        changed_wake_tone_text = self.setting_success_tone_text()
        ...
        default_down_steps = max(int(self.behavior_rules.get("volume", {}).get("default_level", 3) or 3) - 1, 1)
        ...
        work_entry_actions = self.setting_entry_actions(work_entry_word, work_entry_tone_id, work_entry_tone_text)
        ...
        curtain_entry_actions = self.setting_entry_actions(
            curtain_entry_word,
        ...
        wakeword_log_expectations = self.wakeword_log_expectations(changed_wake)
        tick_log_expectations = self.work_mode_log_expectations(tick_word)
        cloth_mode_log_expectations = self.curtain_mode_log_expectations(cloth_mode_word)
        cases = [
            case(
                ...
                entry_tone_text,
                "P1",
                [say_action(entry_word, expect_tone_id=entry_tone_id, expect_tone_text=entry_tone_text, expect_send_protocol=entry_protocol, advisory_log_values=self.config_saved_advisory_logs(), require_protocol_and_tone=False)],
            ...
            case(
                ...
                "恢复出厂后默认唤醒词恢复",
                "设备支持至少一个非默认候选唤醒词",
                f"先把唤醒词设置为【{changed_wake}】，再执行恢复出厂，然后验证默认唤醒词【{self.wake_word}】可用且【{changed_wake}】不再生效",
                "恢复出厂后非默认唤醒词失效，默认唤醒词恢复",
                "",
                "TONE_ID_0",
                self.tone_text("TONE_ID_0"),
                "P1",
                [
                    *wake_entry_actions,
                    say_action(
                        ...
                        advisory_log_values=self.merge_advisory_logs(wakeword_log_expectations, self.config_saved_advisory_logs()),
                        require_protocol_and_tone=False,
                        auto_wake=False,
                    ),
                    say_action(entry_word, expect_tone_id=entry_tone_id, expect_tone_text=entry_tone_text, expect_send_protocol=entry_protocol, advisory_log_values=self.config_saved_advisory_logs(), require_protocol_and_tone=False),
                ...
                [
                    *work_entry_actions,
                    say_action(
                        ...
                        advisory_log_values=self.merge_advisory_logs(tick_log_expectations, self.config_saved_advisory_logs()),
                        require_protocol_and_tone=False,
                        auto_wake=False,
                    ),
                    say_action(entry_word, expect_tone_id=entry_tone_id, expect_tone_text=entry_tone_text, expect_send_protocol=entry_protocol, advisory_log_values=self.config_saved_advisory_logs(), require_protocol_and_tone=False),
            ...
            case(
                ...
                "恢复出厂后默认窗帘模式恢复",
                "设备窗帘模式已被切到非默认模式",
                f"先切到【{cloth_mode_word}】，再执行恢复出厂，然后验证默认窗帘命令【{default_open_word}】恢复生效，纱帘模式命令【{sheer_open_word}】不再响应",
                "恢复出厂后回到默认窗帘/布帘侧模式，纱帘模式命令失效",
                default_open_protocol,
                default_open_tone_id,
                default_open_tone_text,
                "P1",
                [
                    *curtain_entry_actions,
                    say_action(
                        cloth_mode_word,
                        ...
                        advisory_log_values=self.merge_advisory_logs(cloth_mode_log_expectations, self.config_saved_advisory_logs()),
                        require_protocol_and_tone=False,
                        auto_wake=False,
                    ),
                    say_action(entry_word, expect_tone_id=entry_tone_id, expect_tone_text=entry_tone_text, expect_send_protocol=entry_protocol, advisory_log_values=self.config_saved_advisory_logs(), require_protocol_and_tone=False),
    ...
    def build_workmode_cases(self) -> list[dict[str, Any]]:
        ...
        entry_tone_id = norm_text(entry.get("tone_id"), "TONE_ID_9")
        entry_tone_text = norm_text(entry.get("tts_text"), self.tone_text(entry_tone_id))
        entry_actions = self.setting_entry_actions(entry_word, entry_tone_id, entry_tone_text)
        ...
        voice_row = self.word_row(voice_word)
        ...
        voice_log_expectations = self.work_mode_log_expectations(voice_word)
        tick_log_expectations = self.work_mode_log_expectations(tick_word)
        invalid_word = self.recognized_invalid_word("打开窗帘")
        default_open_word = self.first_word(CURTAIN_INTENT_GROUPS["default_open"], "打开窗帘")
        default_open_row = self.word_row(default_open_word)
        ...
        default_open_tone_text = self.row_tone_text(default_open_row, default_open_tone_id, "好的")
        default_open_protocol = norm_hex(default_open_row.get("send_protocol"))
        reentry_actions_from_tick = [
            wake_action(self.wake_word),
            say_action(
                entry_word,
                expect_asr=[entry_word],
                expect_any_tone=True,
                require_protocol_and_tone=False,
                auto_wake=False,
            ),
        ]
        cases = [
            ...
            case(
                self.next_case_id("WORKMODE"),
                "WORKMODE",
                "工作模式设置入口后普通控制词可响应且设置窗口保持",
                "设备处于默认待命态",
                f"先唤醒设备，再进入【{entry_word}】并在设置窗口内说普通控制词【{invalid_word}】",
                "工作模式设置窗口内，普通控制词可正常响应，随后仍可继续完成模式切换",
                "",
                entry_tone_id,
                entry_tone_text,
                "P1",
                [
                    *entry_actions,
                    ...
                    say_action(
                        ...
                        advisory_log_values=self.merge_advisory_logs(voice_log_expectations, self.config_saved_advisory_logs()),
                        require_protocol_and_tone=False,
                        auto_wake=False,
                    ),
                    say_action(
                        ...
                        expect_tone_id=default_open_tone_id,
                        expect_tone_text=default_open_tone_text,
                        advisory_tone_id=default_open_tone_id,
                        advisory_tone_text=default_open_tone_text,
                        expect_send_protocol=default_open_protocol,
                        require_protocol_and_tone=False,
                    ),
                ],
            ...
            case(
                ...
                "工作模式设置超时值符合配置",
                "设备处于默认待命态",
                f"先唤醒设备，进入工作模式设置后不继续选择模式，记录超时事件是否在 {int(timeout_s)}s 左右出现",
                "工作模式设置超时值符合基础配置",
                "",
                "TONE_ID_2",
                self.tone_text("TONE_ID_2"),
            ...
            case(
                ...
                "工作模式设置超时后恢复默认语音模式",
                "设备处于默认待命态",
                f"先唤醒设备，进入工作模式设置后等待实际超时退出，再执行【{default_open_word}】验证仍为默认语音模式",
                "设置超时后退出设置态，普通控制命令仍按默认语音模式响应",
                default_open_protocol,
                default_open_tone_id,
                default_open_tone_text,
                "P1",
                ...
                self.requirement_ids_direct("WORK-001", "WORK-005"),
                "state_transition",
            ),
            case(
                self.next_case_id("WORKMODE"),
                ...
                f"工作模式切换到{tick_word}",
                "设备处于默认待命态",
                f"先唤醒设备，进入【{entry_word}】后切换到【{tick_word}】，再执行普通控制命令【{default_open_word}】检查是否进入滴答模式",
                "切换到滴答模式后，普通控制命令使用滴声反馈",
                default_open_protocol,
                tick_tone_id,
                tick_tone_text,
                "P1",
            ...
            case(
                self.next_case_id("WORKMODE"),
                "WORKMODE",
                f"滴答模式下可重新进入设置并切回{voice_word}",
                "设备处于默认待命态",
                f"先切到【{tick_word}】，再重新唤醒并进入【{entry_word}】切回【{voice_word}】，随后执行【{default_open_word}】检查是否恢复语音模式",
                "工作模式切回语音后，普通控制命令恢复正常语音播报",
                default_open_protocol,
                voice_tone_id,
                default_open_tone_text,
                "P1",
            ...
            case(
                ...
                f"语音模式候选项可被显式确认",
                "设备处于默认待命态",
                f"先唤醒设备，进入【{entry_word}】后显式说【{voice_word}】",
                "语音模式候选项在设置窗口内可被识别并确认",
                norm_hex(voice_row.get("send_protocol")),
                voice_tone_id,
                voice_tone_text,
                "P2",
                [
                    *entry_actions,
                    say_action(
                        voice_word,
                        expect_asr=voice_expect_asr,
                        expect_send_protocol=norm_hex(voice_row.get("send_protocol")),
                        advisory_tone_id=voice_tone_id,
                        advisory_tone_text=voice_tone_text,
                        advisory_log_values=self.merge_advisory_logs(voice_log_expectations, self.config_saved_advisory_logs()),
                        require_protocol_and_tone=False,
                ...
                self.requirement_ids_direct("WORK-003", "DER-WORK-001"),
                "positive",
            ),
        ]
        return cases
    ...
    def build_curtainmode_cases(self) -> list[dict[str, Any]]:
        ...
        entry_tone_id = norm_text(entry.get("tone_id"), "TONE_ID_9")
        entry_tone_text = norm_text(entry.get("tts_text"), self.tone_text(entry_tone_id))
        entry_actions = self.setting_entry_actions(
            entry_word,
            entry_tone_id,
            entry_tone_text,
            expect_log_values={"curtainSettingEntry": ["into set curtain type"]},
        ...
        invalid_word = self.recognized_invalid_word("打开窗帘")
        candidate_map = {norm_text(item.get("semantic")): item for item in candidates if norm_text(item.get("semantic"))}
        default_open_word = self.first_word(CURTAIN_INTENT_GROUPS["default_open"], "打开窗帘")
        default_open_row = self.word_row(default_open_word)
        default_open_tone_id = self.row_tone_id(default_open_row, "TONE_ID_1")
        default_open_tone_text = self.row_tone_text(default_open_row, default_open_tone_id, "好的")
        ...
        invalid_probe_mode_word = "布帘模式" if "布帘模式" in candidate_map else next(iter(candidate_map.keys()), "纱帘模式")
        if invalid_probe_mode_word == "布帘模式":
            invalid_probe_valid_word = cloth_open_word
            invalid_probe_valid_protocol = cloth_open_protocol
            invalid_probe_valid_tone_id = cloth_open_tone_id
            invalid_probe_valid_tone_text = cloth_open_tone_text
        elif invalid_probe_mode_word == "窗纱模式":
            invalid_probe_valid_word = window_open_word
            invalid_probe_valid_protocol = window_open_protocol
            invalid_probe_valid_tone_id = window_open_tone_id
            invalid_probe_valid_tone_text = window_open_tone_text
        else:
            invalid_probe_valid_word = sheer_open_word
            invalid_probe_valid_protocol = sheer_open_protocol
            invalid_probe_valid_tone_id = sheer_open_tone_id
            invalid_probe_valid_tone_text = sheer_open_tone_text

        def mode_case(mode_word: str, valid_word: str, valid_protocol: str, valid_tone_id: str, valid_tone_text: str, invalid_word: str) -> dict[str, Any]:
            mode_row = candidate_map.get(mode_word, self.word_row(mode_word))
            mode_expect_asr = self.accepted_asr_variants(mode_word)
            mode_tone_id = self.setting_success_tone_id()
            mode_tone_text = self.row_tone_text(mode_row, mode_tone_id, self.setting_success_tone_text())
            return case(
                self.next_case_id("CURTAINMODE"),
                "CURTAINMODE",
                f"窗帘模式切换到{mode_word}",
                "设备处于默认待命态",
                f"进入【{entry_word}】后切到【{mode_word}】，随后验证【{valid_word}】可响应且【{invalid_word}】不响应",
                "窗帘模式切换后，仅目标模式对应的控制命令保持生效",
                valid_protocol,
                mode_tone_id,
                mode_tone_text,
                "P1",
                [
                    *entry_actions,
                    say_action(
                        mode_word,
                        expect_asr=mode_expect_asr,
                        advisory_tone_id=mode_tone_id,
                        advisory_tone_text=mode_tone_text,
                        advisory_log_values=self.merge_advisory_logs(
                    ...
                    say_action(valid_word, expect_tone_id=valid_tone_id, expect_tone_text=valid_tone_text, expect_send_protocol=valid_protocol, require_protocol_and_tone=True),
                    say_action(invalid_word, expect_no_response=True, observe_s=2.0),
                ],
                self.requirement_ids_direct("CURTAIN-002", "CURTAIN-004"),
                "state_transition",
            )
        ...
        cases = [
            ...
            case(
                ...
                "",
                "P1",
                [say_action("纱帘模式", expect_no_response=True, observe_s=2.0, forbid_log_values=self.forbid_curtain_mode_change_logs())],
            ...
            case(
                self.next_case_id("CURTAINMODE"),
                "CURTAINMODE",
                "窗帘模式设置入口后普通控制词可响应且设置窗口保持",
                "设备处于默认待命态",
                f"进入【{entry_word}】后，在设置窗口内说普通控制词【{invalid_word}】",
                "窗帘模式设置窗口内，普通控制词可正常响应，随后仍可继续完成模式切换",
                "",
                entry_tone_id,
                entry_tone_text,
                "P1",
                [
                    *entry_actions,
                    ...
                    say_action(
                        invalid_probe_mode_word,
                        expect_asr=self.accepted_asr_variants(invalid_probe_mode_word),
                        advisory_tone_id=self.setting_success_tone_id(),
                        advisory_tone_text=self.setting_success_tone_text(),
                        advisory_log_values=self.merge_advisory_logs(
                            self.curtain_mode_log_expectations(invalid_probe_mode_word),
                            self.config_saved_advisory_logs(),
                        ),
                        require_protocol_and_tone=False,
                        auto_wake=False,
                    ...
                    say_action(
                        invalid_probe_valid_word,
                        expect_tone_id=invalid_probe_valid_tone_id,
                        expect_tone_text=invalid_probe_valid_tone_text,
                        advisory_tone_id=invalid_probe_valid_tone_id,
                        advisory_tone_text=invalid_probe_valid_tone_text,
                        expect_send_protocol=invalid_probe_valid_protocol,
                        require_protocol_and_tone=False,
                    ),
                    *self.restore_default_actions(note="验证完成后恢复默认状态"),
                ],
            ...
            case(
                ...
                "窗帘模式设置超时后恢复默认窗帘模式",
                "设备处于默认待命态",
                f"进入窗帘模式设置后等待实际超时退出，随后验证默认窗帘词【{default_open_word}】仍可用且纱帘词【{sheer_open_word}】不响应",
                "设置超时后退出设置态，窗帘模式不被误改写，仍保持默认窗帘模式",
                default_open_protocol,
                default_open_tone_id,
                default_open_tone_text,
                "P1",
                [
                    *entry_actions,
                    self.wait_for_timeout_exit_action(timeout_s, label="等待窗帘模式设置实际超时退出"),
                    say_action(default_open_word, expect_tone_id=default_open_tone_id, expect_tone_text=default_open_tone_text, expect_send_protocol=default_open_protocol, require_protocol_and_tone=True),
                    say_action(sheer_open_word, expect_no_response=True, observe_s=2.0),
                ],
                self.requirement_ids_direct("CURTAIN-001", "CURTAIN-002", "CURTAIN-006"),
                "state_transition",
            ),
            mode_case("纱帘模式", sheer_open_word, sheer_open_protocol, sheer_open_tone_id, sheer_open_tone_text, cloth_open_word),
            mode_case("窗纱模式", window_open_word, window_open_protocol, window_open_tone_id, window_open_tone_text, cloth_open_word),
            mode_case("布帘模式", cloth_open_word, cloth_open_protocol, cloth_open_tone_id, cloth_open_tone_text, sheer_open_word),
        ]
        return cases
    ...
    def build_curtain_cases(self) -> list[dict[str, Any]]:
        ...
        def synonym_case(label: str, words: list[str]) -> dict[str, Any]:
            primary = self.word_row(words[0])
            protocol = norm_hex(primary.get("send_protocol"))
            tone_id = self.row_tone_id(primary, "TONE_ID_1")
            tone_text = self.row_tone_text(primary, tone_id, "好的")
            actions = self.restore_default_actions(note=f"{label}前恢复默认状态")
            actions.extend(
                [
                    say_action(word, expect_tone_id=tone_id, expect_tone_text=tone_text, expect_send_protocol=protocol, require_protocol_and_tone=False)
                    for word in words[:2]
                ]
            )
            return case(
                ...
                "P1",
                actions,
                self.requirement_ids_direct("DER-CURTAIN-001", "DER-CURTAIN-002"),
                "positive",
            )

        def mode_positive_case(mode_word: str, intent_keys: list[str]) -> dict[str, Any]:
            first_word = self.first_word(CURTAIN_INTENT_GROUPS[intent_keys[0]], CURTAIN_INTENT_GROUPS[intent_keys[0]][0])
            first_row = self.word_row(first_word)
            protocol = norm_hex(first_row.get("send_protocol"))
            tone_id = self.row_tone_id(first_row, "TONE_ID_1")
            tone_text = self.row_tone_text(first_row, tone_id, "好的")
            ...
            actions = [
                *curtain_entry_actions,
                say_action(mode_word, expect_asr=mode_expect_asr, expect_tone_id=mode_tone_id, expect_tone_text=mode_tone_text, require_protocol_and_tone=False, auto_wake=False),
            ]
            for key in intent_keys:
                word = self.first_word(CURTAIN_INTENT_GROUPS[key], CURTAIN_INTENT_GROUPS[key][0])
                row = self.word_row(word)
                actions.append(
                    say_action(
                        word,
                        expect_tone_id=self.row_tone_id(row, "TONE_ID_1"),
                        expect_tone_text=self.row_tone_text(row, "TONE_ID_1", "好的"),
                        expect_send_protocol=norm_hex(row.get("send_protocol")),
                        require_protocol_and_tone=False,
                    )
                )
            return case(
                ...
                f"{mode_word}下控制词有效",
                "设备处于默认待命态",
                f"先切到【{mode_word}】，再依次执行该模式下的开/关/停控制词",
                "目标模式下的开/关/停控制词均可触发正确协议和播报",
                protocol,
                tone_id,
                tone_text,
                "P1",
                actions,
                self.requirement_ids_direct("DER-CURTAIN-003"),
                "state_transition",
            )

        def mode_negative_case(mode_word: str, invalid_word: str) -> dict[str, Any]:
            mode_row = self.word_row(mode_word)
            mode_expect_asr = self.accepted_asr_variants(mode_word)
            mode_tone_id = self.setting_success_tone_id()
            mode_tone_text = self.row_tone_text(mode_row, mode_tone_id, self.setting_success_tone_text())
            return case(
                self.next_case_id("CURTAIN"),
                "CURTAIN",
                f"{mode_word}下非法控制词不响应",
                "设备处于默认待命态",
                f"先切到【{mode_word}】，再执行不匹配当前模式的控制词【{invalid_word}】",
                "不匹配当前模式的控制词不应响应",
                "",
                mode_tone_id,
                mode_tone_text,
        ...
        return [
            synonym_case("默认窗帘打开同义词协议一致", self.curtain_words("default_open", CURTAIN_INTENT_GROUPS["default_open"])),
            synonym_case("默认窗帘关闭同义词协议一致", self.curtain_words("default_close", CURTAIN_INTENT_GROUPS["default_close"])),
            synonym_case("默认窗帘停止同义词协议一致", self.curtain_words("default_stop", CURTAIN_INTENT_GROUPS["default_stop"])),
            mode_positive_case("纱帘模式", ["sheer_open", "sheer_close", "sheer_stop"]),
            mode_positive_case("窗纱模式", ["window_sheer_open", "window_sheer_close", "window_sheer_stop"]),
            mode_positive_case("布帘模式", ["cloth_open", "cloth_close", "cloth_stop"]),
            mode_negative_case("纱帘模式", self.first_word(CURTAIN_INTENT_GROUPS["cloth_open"], "打开布帘")),
            mode_negative_case("窗纱模式", self.first_word(CURTAIN_INTENT_GROUPS["cloth_open"], "打开布帘")),
            mode_negative_case("布帘模式", self.first_word(CURTAIN_INTENT_GROUPS["sheer_open"], "打开纱帘")),
        ]

    def build_phrase_cases(self) -> list[dict[str, Any]]:
        def pair_items(words: list[str]) -> list[dict[str, Any]]:
            return [{"word": word, "accept_asr": list(words)} for word in words]
        ...
        wake_entry_tone_id = norm_text(wake_entry.get("tone_id"), "TONE_ID_9")
        wake_entry_tone_text = norm_text(wake_entry.get("tts_text"), self.tone_text(wake_entry_tone_id))
        setting_success_tone_id = self.setting_success_tone_id()
        setting_success_tone_text = self.setting_success_tone_text()

        def command_phrase_actions(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
            actions: list[dict[str, Any]] = []
            for item in items:
                ...
                if not word:
                    continue
                actions.append(
                    self.positive_command_action(
                        word,
                        accept_asr=list(item.get("accept_asr", [word])),
                        label=f"验证词条【{word}】",
                    )
                )
            return actions
        ...
        def curtain_mode_setup_actions(mode_word: str) -> list[dict[str, Any]]:
            mode_row = self.word_row(mode_word)
            return [
                with_label(wake_action(self.wake_word), f"唤醒设备【{self.wake_word}】"),
                with_label(
                    say_action(
                        curtain_entry_word,
                        expect_asr=[curtain_entry_word],
                        advisory_tone_id=curtain_entry_tone_id,
                        advisory_tone_text=curtain_entry_tone_text,
                        expect_log_values={"curtainSettingEntry": ["into set curtain type"]},
                        require_protocol_and_tone=False,
                        auto_wake=False,
                    ),
                    f"进入窗帘模式设置【{curtain_entry_word}】",
                ),
                with_label(
                    say_action(
                        mode_word,
                        expect_asr=self.accepted_asr_variants(mode_word),
                        advisory_tone_id=setting_success_tone_id,
                        advisory_tone_text=self.row_tone_text(mode_row, setting_success_tone_id, setting_success_tone_text),
                        advisory_log_values=self.merge_advisory_logs(
                            self.curtain_mode_log_expectations(mode_word),
                            self.config_saved_advisory_logs(),
                        ),
        ...
        def wakeword_setting_actions(candidate_words: list[str]) -> list[dict[str, Any]]:
            actions: list[dict[str, Any]] = []
                ...
                actions.extend(
                    [
                        ...
                        with_label(
                            say_action(
                                wake_entry_word,
                                expect_asr=[wake_entry_word],
                                advisory_tone_id=wake_entry_tone_id,
                                advisory_tone_text=wake_entry_tone_text,
                                require_protocol_and_tone=False,
                                auto_wake=False,
                            ),
                            f"进入唤醒词设置【{wake_entry_word}】",
                        ),
                        with_label(
                            say_action(
                                word,
                                expect_asr=[word],
                                advisory_tone_id=setting_success_tone_id,
                                advisory_tone_text=self.row_tone_text(row, setting_success_tone_id, setting_success_tone_text),
                                advisory_log_values=self.merge_advisory_logs(
                                    self.wakeword_log_expectations(word),
                                    self.config_saved_advisory_logs(),
                                ),
                                require_protocol_and_tone=False,
                                auto_wake=False,
        ...
        default_phrase_items = pair_items(self.curtain_words("default_open", CURTAIN_INTENT_GROUPS["default_open"])) + pair_items(
            self.curtain_words("default_close", CURTAIN_INTENT_GROUPS["default_close"])
        ) + pair_items(self.curtain_words("default_stop", CURTAIN_INTENT_GROUPS["default_stop"]))
        cloth_phrase_items = pair_items(self.curtain_words("cloth_open", CURTAIN_INTENT_GROUPS["cloth_open"])) + pair_items(
            self.curtain_words("cloth_close", CURTAIN_INTENT_GROUPS["cloth_close"])
        ) + pair_items(self.curtain_words("cloth_stop", CURTAIN_INTENT_GROUPS["cloth_stop"]))
        sheer_phrase_items = pair_items(self.curtain_words("sheer_open", CURTAIN_INTENT_GROUPS["sheer_open"])) + pair_items(
            self.curtain_words("sheer_close", CURTAIN_INTENT_GROUPS["sheer_close"])
        ) + pair_items(self.curtain_words("sheer_stop", CURTAIN_INTENT_GROUPS["sheer_stop"]))
        window_phrase_items = pair_items(self.curtain_words("window_sheer_open", CURTAIN_INTENT_GROUPS["window_sheer_open"])) + pair_items(
            self.curtain_words("window_sheer_close", CURTAIN_INTENT_GROUPS["window_sheer_close"])
        ) + pair_items(self.curtain_words("window_sheer_stop", CURTAIN_INTENT_GROUPS["window_sheer_stop"]))
        wake_phrase_items = [word for word in self.behavior_rules.get("wake_word_setting", {}).get("candidate_wake_words", [])[:7] if norm_text(word)]
        mixed_phrase_items = [
        ...
        cases.append(
            case(
                ...
                "默认窗帘控制词组检查",
                "设备处于默认窗帘模式",
                f"依次执行默认窗帘开/关/停词组，共 {len(default_phrase_items)} 条",
                "默认窗帘词组中的各个词条均可触发正确播报和协议",
                "",
                "",
                "",
                "P2",
                self.restore_default_actions(note="默认窗帘控制词组检查前恢复默认状态") + command_phrase_actions(default_phrase_items),
        ...
        cases.append(
            case(
                ...
                "布帘控制词组检查",
                "设备处于默认待命态",
                f"先切换到布帘模式，再依次执行布帘开/关/停词组，共 {len(cloth_phrase_items)} 条",
                "布帘词组中的各个词条在布帘模式下均可触发正确播报和协议",
                "",
                "",
                "",
                "P2",
                curtain_mode_setup_actions("布帘模式") + command_phrase_actions(cloth_phrase_items),
                self.requirement_ids_direct("DER-PHRASE-001", "DER-PHRASE-002"),
                "positive",
            )
        )
        cases.append(
            case(
                ...
                "纱帘控制词组检查",
                "设备处于默认待命态",
                f"先切换到纱帘模式，再依次执行纱帘开/关/停词组，共 {len(sheer_phrase_items)} 条",
                "纱帘词组中的各个词条在纱帘模式下均可触发正确播报和协议",
                "",
                "",
                "",
                "P2",
                curtain_mode_setup_actions("纱帘模式") + command_phrase_actions(sheer_phrase_items),
                self.requirement_ids_direct("DER-PHRASE-001", "DER-PHRASE-002"),
                "positive",
            )
        )
        cases.append(
            case(
                ...
                "窗纱控制词组检查",
                "设备处于默认待命态",
                f"先切换到窗纱模式，再依次执行窗纱开/关/停词组，共 {len(window_phrase_items)} 条",
                "窗纱词组中的各个词条在窗纱模式下均可触发正确播报和协议",
                "",
                "",
                "",
                "P2",
                curtain_mode_setup_actions("窗纱模式") + command_phrase_actions(window_phrase_items),
                self.requirement_ids_direct("DER-PHRASE-001", "DER-PHRASE-002"),
                "positive",
            )
        )
        cases.append(
            case(
                ...
                "位置唤醒词组检查",
                "设备处于默认待命态",
                f"依次进入唤醒词设置流程，并验证 {len(wake_phrase_items)} 个位置唤醒词都能被识别为有效设置项",
                "所有位置唤醒词在唤醒词设置流程中都可被正确识别并设置成功",
                "",
                "",
                "",
                "P2",
                wakeword_setting_actions(wake_phrase_items),
                self.requirement_ids_direct("DER-PHRASE-001"),
                "positive",
            )
        )
        cases.append(
            case(
                ...
                "设置与杂项词组检查",
                "设备处于默认待命态",
                f"按语义组连续检查 {len(mixed_phrase_items)} 条词条，而不是孤立单词回放",
                "同组词条能在正确状态下被识别，且在超时后可继续恢复后续检查",
                "",
                "",
                "",
                "P2",
                [phrase_check_action(mixed_phrase_items, self.wake_word)],
                self.requirement_ids_direct("DER-PHRASE-001", "DER-PHRASE-003"),
                "positive",
            )
        )
        return self.isolate_cases(cases)
        ...
        cases = [
            case(
                ...
                [
                    *selector_setup_actions(first),
                    with_label(
                        wake_repeat_action(first, repeats=3, idle_timeout_s=wake_timeout_wait_s),
                        f"连续 3 次使用【{first}】唤醒均成功",
                    ),
                ],
            ...
            case(
                ...
                [
                    *selector_setup_actions(first),
                    with_label(
                        no_wake_action(second, repeats=2),
                        f"验证未设置的候选唤醒词【{second}】不应误唤醒",
                    ),
                ],
            ...
            case(
                ...
                [
                    *selector_setup_actions(second),
                    with_label(
                        no_wake_action(first),
                        f"验证旧定向唤醒词【{first}】不应再唤醒设备",
                    ),
                ],
    ...
    def build_ctrl_cases(self) -> list[dict[str, Any]]:
        ...
        cases = [
            ...
            case(
                ...
                "P1",
                [
                    say_action(entry_word, expect_tone_id=entry_tone_id, expect_tone_text=entry_tone_text, expect_send_protocol=norm_hex(entry.get("send_protocol")), allow_fail_continue=True, require_protocol_and_tone=False),
                ...
                "state_transition",
                blocked_by_case_ids=["TC_CTRL_001"],
            ),
            case(
                "TC_CTRL_003",
                "CTRL",
            ...
            case(
                ...
                "配对窗口超时后注入无效",
                "设备处于默认待命态",
                f"先进入配对窗口，等待 {int(timeout_s)}s 超时退出，再注入配对成功协议，检查窗口外不会再出现 receive msg 或播报",
                ...
                [
                    say_action(
                        ...
                        expect_tone_id=entry_tone_id,
                        expect_tone_text=entry_tone_text,
                        expect_send_protocol=norm_hex(entry.get("send_protocol")),
                        require_protocol_and_tone=False,
                    ),
            ...
            case(
                ...
                "配对成功后窗口关闭",
                "设备处于默认待命态",
                "先进入配对窗口并注入配对成功协议，再次注入协议，检查成功播报后窗口已经关闭",
                ...
                [
                    say_action(
                        ...
                        expect_tone_text=entry_tone_text,
                        expect_send_protocol=norm_hex(entry.get("send_protocol")),
                        allow_fail_continue=True,
                        require_protocol_and_tone=False,
                    ),
                    inject_protocol_action(
                        success_protocol,
                        expect_tone_id=norm_text(success.get("tone_id"), "TONE_ID_7"),
                        expect_tone_text=norm_text(success.get("tts_text"), self.tone_text(norm_text(success.get("tone_id"), "TONE_ID_7")) or "配对成功"),
                        expect_recv_protocol=success_protocol,
                        require_receive_msg=True,
            ...
            case(
                ...
                "配对失败后窗口关闭",
                "设备处于默认待命态",
                "先进入配对窗口并注入配对失败协议，再次注入协议，检查失败播报后窗口已经关闭",
                ...
                [
                    ...
                    inject_protocol_action(
                        failure_protocol,
                        expect_tone_id=norm_text(failure.get("tone_id"), "TONE_ID_8"),
                        expect_tone_text=norm_text(failure.get("tts_text"), self.tone_text(norm_text(failure.get("tone_id"), "TONE_ID_8")) or "配对失败"),
                        expect_recv_protocol=failure_protocol,
                        require_receive_msg=True,
                        requires_active_window=True,
                    ),
...
def render_case_steps(actions: list[dict[str, Any]]) -> str:
    return "\n".join(f"{index}. {action_text(action)}" for index, action in enumerate(actions, 1))
