...
class DooyaRunner:
    ...
    def archive_workspace_artifacts(self) -> None:
        ...
        for name, src in sources.items():
            if src.exists():
                copy_into(src, archive_root / name)
    ...
    def validate_protocol_serial(self) -> bool:
        ...
        for attempt in range(attempts):
            ...
            if expected:
                matched = any(expected in item for item in protocols)
            if matched:
                break
        ...
        if matched:
            reason = f"播放 {word} 后已观察到协议串口数据"
            if result.get("status") != "PASS" and detail:
                reason += f"（动作摘要：{detail}）"
        elif result.get("status") != "PASS" and detail:
            reason = f"播放 {word} 后未稳定观察到协议串口数据：{detail}"
        else:
            reason = f"播放 {word} 后未观察到协议串口数据"
        if matched and expected and not any(expected in item for item in protocols):
            reason += f"；最近协议与期望 {expected} 不完全一致，实际为 {'/'.join(protocols[-3:]) or '-'}"
    ...
    def run_say(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        auto_wake = bool(action.get("auto_wake", True)) and not expect_no_response
    ...
    def run_inject_protocol(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        for attempt in range(1, retries + 1):
            ...
            recv_values = [normalize_hex(item) for item in self.reader.get_all("recvMsg")] if self.reader else []
            ...
            if partial and not frames:
                ...
                return self._make_action_result(
                    ...
                    start,
                    actual_tones=tones,
                    actual_protocols=frames + recv_values,
