...
def build_dynamic_blocks(req: dict[str, Any], reg: dict[str, Any], words: dict[str, dict[str, str]], wake_words: list[str]) -> dict[str, list[list[str]]]:
    ...
    return {
        ...
        "4.7 配置与参数一致性专项": [
            ...
            ["CFG-VOL-001", "配置一致性-音量参数", "正例", f"默认音量应为 `{req['default_volume']}`", "已清配置或完成基线重启", "1. 上电启动<br>2. 读取启动配置中的 `volume` 字段", f"默认音量字段应为 `{req['default_volume']}`", "`COM38` 启动配置中的 `volume` 值与需求一致"],
            ...
            ["REG-CFG-005", "配置一致性-语音注册", "正例", f"命令词模板数上限 = `{reg['command_template_count']}`", "已完成命令词学习并继续尝试学习", "1. 连续学满命令词模板<br>2. 再次进入对应学习流程", f"达到 `{reg['command_template_count']}` 个模板后，应出现模板已满 / 学习结束提示", "辅助看 `regCmdCount`、`reg over!` 等日志"],
...
def render_case_markdown(existing_text: str, req: dict[str, Any], reg: dict[str, Any], words: dict[str, dict[str, str]], wake_words: list[str]) -> str:
    ...
    rendered += f"- 当前状态分布：`PASS={counter.get('PASS', 0)}`、`FAIL={counter.get('FAIL', 0)}`、`TODO={counter.get('TODO', 0)}`\n"
