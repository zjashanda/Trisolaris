...
DEVICE_KEY = env_text("TRISOLARIS_DEVICE_KEY", DEFAULT_DEVICE_KEY)
...
_result_root_text = env_text("TRISOLARIS_BATCH_RESULT_ROOT", str(ROOT / "result"))
_result_root_path = Path(_result_root_text).expanduser()
...
def main() -> int:
    ...
    run_branch(
        ...
        lambda: (
            ...
            runner.run_dual("reg_tc_learn_cmd_close_save_closure", ["小度小度", "笑逐颜开"], post_wait_s=8.0),
    ...
    run_branch(
        ...
        lambda: (
            ...
            runner.run_dual("reg_voice005_cmd_retry_exhaust_failed_alias_probe", ["小度小度", "万事大吉"], post_wait_s=6.0),
        ),
    )
    ...
    run_branch(
        ...
        lambda: (
            runner.run_dual("reg_voice007_cmd_conflict_volume_word_sequence", ["小度小度", "学习命令词", "大声点", "大声点"]),
            runner.run_dual("reg_voice007_cmd_conflict_volume_word_recheck", ["小度小度", "大声点"]),
        ),
    )
    ...
    run_branch(
        ...
        lambda: (
            ...
            runner.run_dual("reg_voice013_delete_wakeup_confirm_sequence", ["小度小度", "删除唤醒词", "删除唤醒词"], post_wait_s=8.0),
            runner.run_shell("reg_voice013_delete_wakeup_confirm_reboot_after_delete", "reboot", capture_s=10.0, ready_wait_s=8.0),
