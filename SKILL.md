...
9. raw FAIL 若来自断言逻辑、空采集、状态污染、保存闭环缺失、前置条件未闭合、TTS 短语误选、端口映射漂移、注入时机过早或 MCU 回包缺失，必须先修验证路径并重跑。
10. 最终 FAIL 只能保留两类：固件不满足需求，或需求本身错误/矛盾。
11. 新需求或新变体收敛后，把通用逻辑回灌 `references/validation-pool/`，项目特定结果同步到项目用例资产和 `plan.md`。
